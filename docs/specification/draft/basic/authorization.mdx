...
3. **Use URL Query Parameter** (Alternative **REQUIRED**, most compatible, moderate security risk):
   ```javascript
   const ws = new WebSocket(`wss://mcp.example.com/mcp?access_token=${encodeURIComponent(token)}`);
   ```

4. **Use WebSocket Subprotocol** (Alternative, simplest, limited token length, most like HTTP Bearer header):
   ```javascript
   const ws = new WebSocket('wss://mcp.example.com/mcp', [`bearer.${token}`]);
   ```
...
- **Use Query Parameters** when:
  - Token is short enough for URL inclusion
  - Simple implementation and debugging is preferred
  - Cookie support is unavailable

- **Use Subprotocols** when:
  - Token is short (typically less than 100 characters)
  - Simple protocol negotiation is desired
  - Server is adapted from HTTP Bearer token implementation
...
- Access tokens **MUST NOT** be transmitted in WebSocket message frames after connection establishment
- Client **MUST** support at least the URL query parameter method
...
- Non-browser clients **MAY** transmit access tokens in an HTTP Bearer header (not shown above) during WebSocket upgrade
