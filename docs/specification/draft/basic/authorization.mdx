...
4. **Use WebSocket Subprotocol** (Alternative, simplest, limited token length, most like HTTP Authorization: Bearer header):
   ```javascript
...
- Non-browser clients **MAY** transmit access tokens in an HTTP Authorization: Bearer header (not shown above) during WebSocket upgrade
