...
This transport provides long-lived bidirectional communication over a WebSocket connection which provides better support server-initiated notifications like `resources/subscribe` and `list_changed` enabling real-time usage scenarios.
...
1. A server using the WebSocket transport **MUST** assign an MCP session ID at
   initialization time, by including a `result.mcpSessionId` field in the JSON-RPC response.
   - The session ID **SHOULD** be globally unique and cryptographically secure (e.g., a
     securely generated UUID, a JWT, or a cryptographic hash).
   - The session ID **MUST** only contain visible ASCII characters (ranging from 0x21 to
     0x7E).
...
- Regardless, receipt of the first message for an existing session over a different connection, the server **MUST** associate this new connection with previously established session state making this the only active connection for the session
...
The only way to communicate errors at the WebSocket protocol level is to terminate the connection with a close code and an optional reason. These errors **MUST NOT** result in the termination of the MCP session. After such an error, the client is free to reconnect using an active `mcpSessionId` to resume normal operation. These include:
