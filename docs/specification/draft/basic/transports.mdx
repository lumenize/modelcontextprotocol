...
A key motivation for this WebSocket transport is to enable bi-directional usage including server to client notifications, which is unworkable without session state that survives connection disruptions. Therefore, servers and clients using this WebSocket transport **MUST** provide MCP session support.

1. A server using the WebSocket transport **MUST** assign an MCP session ID at
   initialization time, by including it a `result.mcpSessionId` field in the JSON-RPC response.
   - The session ID **SHOULD** be globally unique and cryptographically secure (e.g., a
     securely generated UUID, a JWT, or a cryptographic hash).
   - The session ID **MUST** only contain visible ASCII characters (ranging from 0x21 to
     0x7E).
2. Clients using the WebSocket transport **MUST** include the `params.mcpSessionId` field
   in all of their subsequent WebSocket messages (requests and notifications).
   - Servers **SHOULD** respond to requests without a `params.mcpSessionId` field (other than initialization) with a JSON-RPC error code `-32602` and message "Missing mcpSessionId".
3. The server **MAY** terminate the session at any time, after which it **MUST** respond
   to requests containing that `mcpSessionId` with a JSON-RPC error code `-32602` and message "Invalid mcpSessionId".
4. The server **MUST** include a `params.mcpSessionId` field in all of its WebSocket
...
Upon reconnection:
- Client **MAY** include one `mcp.session-id.${mcpSessionId}` value in the WebSocket subprotocols list via the `Sec-WebSocket-Protocol` header in the WebSocket upgrade request to help with loadbalancer affinity
...
- Regardless, receipt of the first message for an existing session over a different connection, the server **MUST** associate this new connection with previously established session state
...
Note: The only way to communicate errors at this level is to terminate with a close code and an optional reason. These errors **MUST NOT** result in the termination of the MCP Session. After such an error, the client is free to reconnect using an active `mcpSessionId` to resume normal operation.
