...
### MCP Session Support Required For WebSocket Transport
...
- Servers and clients using this WebSocket transport **MUST** provide session support. Rationale: In Streamable HTTP transport, session IDs are used for update subscription management when using the streaming/SSE channel. It is optional for stateless HTTP use since the "session" only lasts for duration of a single request/response cycle. However, the motivation for this WebSocket transport is to enable bi-directional usage which is impossible without session support.
- The relationship of MCP sessions to WebSocket connections is many to one, so multiple sessions to share **MAY** share the same connection.
- Subscriptions are associated with a particular session so server-initiated notifications **MUST** only be sent over the active connection for that session.
...
**Client Behavior:**
- Sends HTTP GET WebSocket upgrade request including auth token in WebSocket subprotocol list (see [Authorization](#authorization) for details) [TODO: Update this if necessary once authorization for WebSockets is finalized]
- Receive acceptance or rejection response
- If accepted, establish MCP session by sending `initialize` without `sessionId` [TODO: Update this once SEP-1364 is finalized]

**Server Behavior:**
- Receive HTTP GET WebSocket upgrade request
- If auth fails, reject the upgrade request
- If auth succeeds, accept WebSocket connection
...
Upon reconnection:
- Client **MUST** include the `sessionId` from the previous session in any message after reconnection
- Server **MUST** recognize the existing `sessionId` and associate this new connection with previously established session state
- Server **MUST** disassociate any previous WebSocket connection associated with the same MCP `sessionId` and ignore any messages that arrive for the MCP session on any connection but the active one
...
**Warning: MCP `sessionId` is not a security mechanism**. It does not provide authentication or authorization. This specification defines MCP session management for reconnection, which is orthogonal to user authentication and authorization.
