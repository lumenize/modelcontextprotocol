...
Upon reconnection:
- Client **MUST NOT** send an `initialize` message when reconnecting over a different WebSocket connection to an existing session
- Client **MUST** include the `mcpSessionId` from the previous session in any message after reconnection
- Upon receipt of the first message for an existing session over a different connection, the server **MUST** associate this new connection with previously established session state
